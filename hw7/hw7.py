...
def solve_laplacian(u, u_cond, h, Niter=5000):
    ...
    for i in range(Niter):
        ...
        V[1:-1, 1:-1] = (V[0:-2, 1:-1] + V[2:, 1:-1] + V[1:-1, 0:-2] + V[1:-1, 2:])/4
    return V
...
Q = 0
# [0, 0] is in bottom left, first index is in horizontal
plate_left = int(N/2) - int(L/h/2) - 5
plate_right = int(N/2) + int(L/h/2) + 5
plate_bottom = int(N/2) + int(d/h/2) - 5
plate_top = int(N/2) + int(d/h/2) + 5
# plate_left = int(N*(1/4))
# plate_right = int(N*(3/4))
# plate_bottom = int(N*(2/4))
# plate_top = int(N*(3/4))
...
C_ideal = epsilon*L/d
print(f"Ideal     Capacitance: {C_ideal}")
print(f"Non-ideal Capacitance: {C_nonideal}")
print(f"Non-ideal Difference:  {C_nonideal/C_ideal-1:.2%}")
